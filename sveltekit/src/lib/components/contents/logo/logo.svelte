...
{#if withTitle || withSubTitle}
	<div class={cn('@container aspect-4/1', className)}>
		<div class={cn('flex flex-row items-center justify-center gap-[3cqw]', className)}>
			{@render SVGLogo({
				visible,
				className: 'w-1/4 origin-bottom-right -translate-y-[2cqw]'
			})}
			{#if withTitle}
				<div class="flex flex-col items-start justify-center gap-[2.5cqw]">
					<h1
						class={cn(
							'font-cal-sans text-[17cqw] leading-none',
					...
					{#if withSubTitle}
						<p class="flex flex-row items-center gap-[2.5cqw] font-cal-sans text-[4.5cqw]">
							<span>plan</span>
							<span>&#9679;</span>
							<span>do</span>
							<span>&#9679;</span>
...
{:else}
	{@render SVGLogo({ visible, className: cn('aspect-[143/179]', className) })}
{/if}

{#snippet SVGLogo({ visible, className }: { visible: boolean; className: ClassValue | null })}
	<svg class={className} viewBox="0 0 143 179" fill="none" xmlns="http://www.w3.org/2000/svg">
		{#if visible}
			<path
